...
describe("ManagedPeriodicAggregatorOracle#setTokenConfig", function () {
    var oracle;
    ...
    beforeEach(async function () {
        ...
        newValidationStrategy = await validationStrategyFactory.deploy(DEFAULT_QUOTE_TOKEN_DECIMALS, 0, 0, 1, 100000);
    ...
    it("Reverts if the new config is missing oracles", async function () {
        ...
        await expect(oracle.setTokenConfig(GRT, alternativeTokenConfigWith0Oracles.address)).to.be.revertedWith(
            'InvalidTokenConfig("' + alternativeTokenConfigWith0Oracles.address + '", ' + ERROR_MISSING_ORACLES + ")"
        );
    });
    ...
    it("Reverts if the new config has duplicate oracles", async function () {
        ...
        await expect(oracle.setTokenConfig(GRT, alternativeTokenConfigWithDuplicateOracles.address)).to.be.revertedWith(
            'InvalidTokenConfig("' +
...
describe("ManagedPeriodicAggregatorOracle#update", function () {
    var oracle;
    var aggregationStrategy;
    var validationStrategy;
    ...
    it("Uses the new token config's validation strategy", async function () {
        ...
        const tokenConfigFactory = await ethers.getContractFactory("OracleAggregatorTokenConfig");
        const newTokenConfig = await tokenConfigFactory.deploy(aggregationStrategy, newValidationStrategy.address, 1, [
            underlyingOracle1.address,
        ]);
...
describe("ManagedPeriodicAggregatorOracle#supportsInterface(interfaceId)", function () {
    var oracle;
    var interfaceIds;
