...
describe("AaveV3SupplyMutationComputer#constructor", function () {
    var factory;
    var aclManager;
    var lendingPool;

    beforeEach(async function () {
        factory = await ethers.getContractFactory("AaveV3SupplyMutationComputerStub");
...
describe("AaveV3SupplyMutationComputer#getValue", function () {
    var computer;
    var aclManager;
    var lendingPool;
    var token;
    ...
    function describeTests(decimals, decimalsOffset) {
        for (const test of commonTests) {
            ...
            it(`Works with a value of ${test.toString()}, and the result is ${expected.toString()}`, async function () {
                ...
                collateralToken = await erc20Factory.deploy("AToken", "aToken", decimals);
                await collateralToken.deployed();

                const [owner] = await ethers.getSigners();

                // Mint test amount of tokens
                await collateralToken.mint(owner.address, test);

                // Configure the lending pool to use the token
                await lendingPool.setCollateralToken(token, collateralToken.address);
            ...
            for (const decimalsOffset of decimalOffsetsToTest) {
                describe(`Using a decimals offset of ${decimalsOffset}`, function () {
                    describe("Using a token with 18 decimals", function () {
                        beforeEach(async function () {
                            const computerFactory = await ethers.getContractFactory("AaveV3SupplyMutationComputerStub");
...
describe("AaveV3SupplyMutationComputer#setConfig", function () {
    var aclManager;
    var lendingPool;
    var computer;
...
describe("AaveV3SupplyMutationComputer#supportsInterface", function () {
    var interfaceIds;
    var computer;
