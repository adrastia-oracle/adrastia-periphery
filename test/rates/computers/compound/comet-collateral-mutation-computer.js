...
describe("CometCollateralMutationComputer#constructor", function () {
    var factory;

    var comet;

    beforeEach(async function () {
        factory = await ethers.getContractFactory("CometCollateralMutationComputer");
...
describe("CometCollateralMutationComputer#getValue", function () {
    var computer;
    var token;
    var comet;
    ...
    function describeTests(decimals, decimalsOffset) {
        for (const amount of commonTests) {
            ...
            it(`Works with a collateral supply of ${amount.toString()}, and the result is ${expected.toString()}`, async function () {
                await comet.stubSetTotalsCollateral(token, amount);
                ...
                describe(`Using a decimals offset of ${decimalsOffset}`, function () {
                    describe("Using a token with 18 decimals", function () {
                        beforeEach(async function () {
                            ...
                            const computerFactory = await ethers.getContractFactory(
                                "CometCollateralMutationComputerStub"
                            );
    ...
    it("Reverts if the token address is zero", async function () {
        ...
        const computerFactory = await ethers.getContractFactory("CometCollateralMutationComputerStub");
        computer = await computerFactory.deploy(
            comet.address,
            DEFAULT_ONE_X_SCALAR,
            DEFAULT_DECIMALS,
...
describe("CometCollateralMutationComputer#setConfig", function () {
    var computer;
...
describe("CometCollateralMutationComputer#supportsInterface", function () {
    var interfaceIds;
    var computer;
    ...
    beforeEach(async function () {
        ...
        const computerFactory = await ethers.getContractFactory("CometCollateralMutationComputer");
        computer = await computerFactory.deploy(
            comet.address,
            DEFAULT_ONE_X_SCALAR,
            DEFAULT_DECIMALS,
