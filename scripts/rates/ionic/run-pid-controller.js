...
const MODE_IONIC_COMPTROLER = "0xfb3323e24743caf4add0fdccfb268565c0685556";
...
async function main() {
    ...
    const inputPrecisionDecimals = 18;
    const outputPrecisionDecimals = 18;
    const deltaTerm = 20;
    const errorScalarNumerator = BigNumber.from(2375);
    const errorScalarDenominator = BigNumber.from(1000);

    const startingRate = ethers.utils.parseUnits("0.08", outputPrecisionDecimals); // 8% APY
    var target = ethers.utils.parseUnits("0.85", inputPrecisionDecimals);
    var input = ethers.utils.parseUnits("0.40", inputPrecisionDecimals);

    // The following configuration assumes that 1e18 = 100% for rates

    // The maximum rate
    const max = ethers.utils.parseUnits("0.25", outputPrecisionDecimals); // 25%
    // The minimum rate
    const min = ethers.utils.parseUnits("0.00", outputPrecisionDecimals); // 0%
    // The maximum increase in the rate per update
    const maxIncrease = ethers.utils.parseUnits("0.20", outputPrecisionDecimals); // 20%
    // The maximum decrease in the rate per update
    const maxDecrease = ethers.utils.parseUnits("0.05", outputPrecisionDecimals); // 5%
    ...
    const contractName = "IonicPidController";
    const factory = await ethers.getContractFactory(contractName);
    const rateController = await factory.deploy(
        MODE_IONIC_COMPTROLER,
        oracle.address,
        period,
